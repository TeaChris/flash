...
const getRedisConnectionOptions = (): ConnectionOptions | null => {
  try {
    ...
    if (!redisUrl) {
      logger.warn(
        'Redis URL not configured. BullMQ functionality will be disabled.'
      )
      return null
    }
...
export const addJob = async (
  ...
  try {
    ...
    return (
      job.id?.toString() ??
      `error:${queueName}:${Date.now()}:${Math.random()
        .toString(36)
        .substring(2, 10)}`
    )
  } catch (error) {
...
export const createWorker = (
  ...
  try {
    ...
    const worker = new Worker(
      ...
      async (job) => {
        ...
        try {
          ...
          logger.error(
            `Error processing job ${job.id} from queue ${queueName}:`,
            error
          )
    ...
    logger.info(
      `Worker created for queue ${queueName} with concurrency ${concurrency}`
    )
    return worker
  } catch (error) {
